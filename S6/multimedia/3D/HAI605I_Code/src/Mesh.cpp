...
void Mesh::smooth( float lambda ){
    ...
    vector<vector<unsigned int>> coords;
    this->collectOneRing(coords);
    ...
    for (size_t i = 0; i < coords.size(); i++) {
        ...
        for (size_t j = 0; j < coords[i].size(); j++) {
            ...
            PosMoy[i] += vertices[coords[i][j]].position;
        }
    }
